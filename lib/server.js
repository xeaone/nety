...
module.exports = class Servey extends Events {
	...
	async handler (request, response) {
		...
		if (context.url.pathname !== '/' && context.url.pathname.slice(-1) === '/') {
			...
			return await self.ender(context, result);
		}
		...
		if (self.event && self.event.handler && typeof self.event.handler.before === 'function') {
			await self.event.handler.before(context);
		}

		const result = await route.handler(context);

		if (self.event && self.event.handler && typeof self.event.handler.after === 'function') {
			await self.event.handler.after(context);
		}

		await self.ender(context, result);
	}
