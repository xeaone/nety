...
module.exports = class Servey extends Events {
    ...
    async streamer (context) {
        return new Promise(function (resolve, reject) {
            let stream = context.body;

            if (!context.request.headers.range) {
                const encoding = context.request.headers['accept-encoding'];
            ...
            stream.pipe(context.response).on('end', resolve).on('error', reject);
        });
    }
    ...
    async ender (context) {
        ...
        if (context.body instanceof Stream.Readable) {
            ...
            console.log(context.request.headers.range);
    ...
    async handler (request, response) {
        ...
        const tool = Object.assign(self.tool);
        ...
        if (context.method.includes('OPTIONS')) {
            const security = await self.tool.head.security(context, {});

            context.code = 204;
            context.head = Object.assign(context.head, security);
            context.response.writeHead(context.code, context.head);
